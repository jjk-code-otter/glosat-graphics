...
fig.set_size_inches(xsize, 10)
fig.tight_layout()
fig.subplots_adjust(left=0.0, bottom=0.0, right=1, top=1)
plt.plot([0, xsize], [0, 10], color=None, linewidth=0)

for x, y in itertools.product(range(xsize), range(10)):

    lat_factor = 0.5

    low = 0.8 * (1 - lat_factor + lat_factor * np.sin(np.pi * y / 10.)) - x / (xsize-1)
